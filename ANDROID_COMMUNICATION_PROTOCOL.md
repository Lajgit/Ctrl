...
投币器脉冲输入为 PE15，每个完整有效脉冲固定计为人民币 1 元，即 `100` 分。

## 5. 金额单位约定

控制板与安卓之间的价格、余额全部使用**人民币分**表示，禁止发送浮点数、IEEE 754 数据或十进制字符串。
...
安卓设置价格时，应先将元转换成分，再将整数按大端顺序写入 `Data1:Data4`：

| 单颗价格 | Data1:Data4 整数 | 十六进制 |
|---:|---:|---|
| 0.01 元 | 1 | `0x00000001` |
| 1.00 元 | 100 | `0x00000064` |
| 100.00 元 | 10000 | `0x00002710` |

允许价格范围为 **0.01～100 元/颗**，即协议整数范围 `1～10000` 分。
...
| `0x20` | BeadPriceSet | Data1:Data4 = 单颗价格（分） | 设置单颗珠子价格，有效范围 `1～10000` 分，即 0.01～100 元/颗 |
...
- `ExpandCode=0x01`：价格无效，Data1:Data4 返回当前有效价格，单位仍为分。
...
|---|---|---|---|---|
| `0x00` | VersionRequest | `0xMMNNPPBB` | 需要 | 固件版本，当前价格分单位版本为 1.1.1.0 |
...
| `0x20` | BeadPriceStatus | Data1:Data4=单颗价格（分） | 不需要 | 当前单颗价格；ExpandCode 表示价格设置结果 |
| `0x21` | BeadStockStatus | Data1:Data4=当前库存 | 不需要 | 每成功吐出一颗后同步最新库存 |
| `0x22` | PurchasePendingStatus | Data1:Data4=欠吐数量 | 不需要 | 已收款但尚未成功吐出的珠子数量 |
| `0x23` | PurchaseCreditStatus | Data1:Data4=累计余额（分） | 不需要 | 尚不足一颗珠子的人民币余额 |
...
1. 安卓通过 `BeadPriceSet` 设置单颗珠子价格，单位为分；首次使用默认价格为 100 分，即 1.00 元/颗。
2. 硬币机每个有效脉冲累计 100 分。
3. 纸币机在收到正常堆叠 `0x10` 或卡钞已堆叠 `0x83` 后，按 `0x40~0x45` 映射累计金额。
4. 控制板按 `累计金额（分） / 单颗价格（分）` 计算新增待吐数量，不能整除的余额继续以分保存。
...
示例：单颗价格为 2.50 元时，安卓发送 `250`。投入 5 元纸币后固件累计 `500` 分，计算得到 `500 / 250 = 2` 颗。
...
- 保存内容：单颗价格（分）、库存、已付款欠吐数量、累计余额（分）、无珠锁定状态；
...
- V1：价格和余额保存为整数元；
- V2：价格和余额保存为整数分；
- 固件 1.1.1.0 能读取 V1 记录，将元乘以 100 转换成分，再追加保存为 V2；
- 若旧价格转换后超过 100.00 元，迁移时限制为 10000 分。
...
- 安卓是否严格以分发送和解析 `BeadPriceSet`、`BeadPriceStatus`、`PurchaseCreditStatus`；
- 0.01 元、1.00 元、2.50 元、100.00 元四个价格边界与典型值；
