...
/**
 * 服务端 SDK 的 MQTT 凭证安全存储实现。
 *
 * <p>完整凭证快照先序列化，再使用 AndroidKeyStore 中的 AES-GCM 密钥加密，
 * 最后通过一次 SharedPreferences commit 原子替换。任何字段缺失、解密失败或
 * Topic 校验失败都返回 null，不拼接新旧凭证。</p>
 */
public final class SdkCredentialStore implements CredentialStore {

    private static final String PREF = "pinball_sdk_credential_v1";
    private static final String KEY_BLOB = "credential_blob";
    private static final String KEY_ALIAS = "com.gouzhu.sdk.mqtt.credential.aes.v1";
    ...
    public synchronized MqttCredential load() {
        ...
        try {
            ...
            // 构造函数会再次执行 deviceNo、Topic、QoS 和完整字段校验。
    ...
    public synchronized void replaceAtomically(MqttCredential credential) {
        if (credential == null) {
            throw new IllegalArgumentException("MQTT凭证不能为空");
        }
        ...
        try {
            ...
            if (!preferences().edit().putString(KEY_BLOB, blob).commit()) {
                throw new IllegalStateException("MQTT凭证原子保存失败");
            }
        } catch (RuntimeException error) {
            throw error;
        } catch (Throwable error) {
            throw new IllegalStateException("MQTT凭证加密保存失败", error);
        }
    }
    ...
    public synchronized void clear() {
        if (!preferences().edit().clear().commit()) {
            throw new IllegalStateException("MQTT凭证清理失败");
        }
    }
